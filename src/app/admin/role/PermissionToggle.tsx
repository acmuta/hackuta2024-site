...
export function PermissionToggle({
  ...
  const ToggleButton = ({
    ...
    <button onClick={() => onChange?.(valueToSet)} title={valueToSet}>
      <Icon
        className={
          value === valueToSet ? twJoin(className, "text-white") : `text-black`
        }
  ...
  return (
    <div className={twJoin("flex flex-col")}>
      <div
        className={twJoin("flex justify-between gap-12 border-b border-black")}
      >
      ...
      {value === "Partial" && (
        <div className="flex flex-col pl-8">{children}</div>
      )}
    </div>
  )
}
