...
export default function ApplicantDataTable({
  ...
  const isBlocked = (r: Row) =>
    blockedHackers.some(
      (b) => `${b.first_name} ${b.last_name}` === `${r.firstName} ${r.lastName}`
    )
  ...
  return (
    <div className="flex flex-col gap-2">
      <p>
        Tot/Acc/Rej/Wai/Und: {applications.length}/{counts.accepted ?? 0}/
        {counts.rejected ?? 0}/{counts.waitlisted ?? 0}/{counts.undecided ?? 0}.{" "}
        <a
